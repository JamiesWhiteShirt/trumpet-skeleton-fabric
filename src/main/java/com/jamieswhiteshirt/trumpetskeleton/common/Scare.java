...
public class Scare {
    public static void scare(World world, LivingEntity user) {
        if (!world.isClient) {
            List<LivingEntity> entities = world.getEntities(LivingEntity.class, user.getBoundingBox().expand(5.0D));
            for (LivingEntity spookedEntity : entities) {
                if (spookedEntity == user) continue;
                double deltaX = spookedEntity.x - user.x + world.random.nextDouble() - world.random.nextDouble();
                double deltaZ = spookedEntity.z - user.z + world.random.nextDouble() - world.random.nextDouble();
